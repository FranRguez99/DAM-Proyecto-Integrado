...
class QuizController(context: Context) {
    ...
    fun getNextQuestion(): List<Pais> {
        currentQuestionIndex++
        if (currentQuestionIndex >= listPaises.size) {
            currentQuestionIndex = 0
        }
        return getNewQuestion()
    }

    fun checkAnswer(selectedPais: Pais) {
        if (currentQuestion.contains(selectedPais)) {
            rightAnswerList.add(selectedPais)
        } else {
            wrongAnswerList.add(selectedPais)
        }
    }

    fun hasMoreQuestions(): Boolean {
        return listAnsweredQuestion.size < listPaises.size
    }
}
